...
#ifndef YUNPAI_UTIL_H_
#define YUNPAI_UTIL_H_
...
std::string ByteArrayToHexStr(uint8_t const* bytes, uint32 length, bool reverse = false);
...
public:
    ...
    inline uint32 & operator [](uint8_t el)
    {
        return part[el];
    }

    inline const uint32 & operator [](uint8_t el) const
    {
        return part[el];
    }
};
